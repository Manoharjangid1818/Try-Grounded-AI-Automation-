...
const conversationAnalysisData = readJsonFile('./data/conversationAnalysis.data.json');
...
test.describe('Conversation Analysis Tests @conversation @regression', () => {
    for (const data of conversationAnalysisData) {
        test(`${data.testCaseId} - ${data.testCaseName} @conversation @regression`, async ({
            page
        }, testInfo) => {
            ...
            if (data.scenarioType === 'pageOpen') {
                ...
                await conversationAnalysisPage.enterConversationName(data.conversationName);
            } else if (data.scenarioType === 'fileUpload') {
                await conversationAnalysisPage.uploadReferenceDocument(data.referenceFilePath);
                ...
                await conversationAnalysisPage.verifyEmptyTranscriptValidation(data, runAttempt);
